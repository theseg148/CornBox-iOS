...
final class V8Action: UIControl {
    ...
    init(icon: String, label: String) {
        ...
        count.font = .systemFont(ofSize: 10, weight: .bold)
        let s = UIStackView(arrangedSubviews: [self.icon, text, count])
        s.axis = .vertical
        s.spacing = 2
        s.alignment = .center
        ...
        addSubview(s)
        NSLayoutConstraint.activate([s.leadingAnchor.constraint(equalTo: leadingAnchor),s.trailingAnchor.constraint(equalTo: trailingAnchor),s.topAnchor.constraint(equalTo: topAnchor),s.bottomAnchor.constraint(equalTo: bottomAnchor),self.icon.widthAnchor.constraint(equalToConstant: 30),self.icon.heightAnchor.constraint(equalToConstant: 30)])
        count.isHidden = true
